...
def parse_vocal_parts_from_root(root_el: ET.Element | XmlNode) -> List[MusicXMLPart]:
    root: XmlNode = (create_xml_node(cast(ET.Element, root_el))
    ...
    for part_idx, (part_name, part_node) in enumerate(zip(part_names, parts)):
        ...
        voices: List[str] = list(set([v.text
...
def merge_nodes(
    ...
    for co_exclusive_group in co_exclusive_tags:
        ...
        new_children = [c for c in new_children
...
def merge_print_nodes(
    curr_node: XmlNode | None,
    node_or_nodes_to_merge: XmlNode | List[XmlNode,]
):
    '''
    In this order
    ...
    node_to_merge = cast(XmlNode,
                         node_or_nodes_to_merge[-1] if type(node_or_nodes_to_merge) == list else node_or_nodes_to_merge)
    ...
    return merge_nodes(curr_node=curr_node, new_node=node_to_merge, tags_to_merge=tags_to_merge)
...
def merge_attributes_nodes(
    ...
    return merge_nodes(curr_node=curr_node,
