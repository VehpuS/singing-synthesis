import { Dictionary, compact, filter, findLast, findLastIndex, flatMap, forEach, includes, last, map, reduce, slice, some, sortBy, times } from "lodash";
...
const parseTempoChangesFromMeasureChild = ({
    ...
    return {
        newTempoEvents: tempoEvents, newTempo: currentTempo, newDivisions: currentDivisions
    };
}
...
export const musicXMLToEvents = (
    ...
    for (let measureIdx = 0; measureIdx < numMeasures; measureIdx++) {
        ...
        const partHasTempoChangesInMeasure = map(
            allPartsMeasures,
            (partMeasures) => {
        ...
        const partsOrderedByTempoChanges = sortBy(times(partNames.length), (partIdx) => partHasTempoChangesInMeasure[partIdx] ? 0 : 1);
        for (const partIdx of partsOrderedByTempoChanges) {
            ...
            let currentDivisions = findLast(
                tempoEvents,
                (e) => e.partIdx === partIdx && e.measureIdx === measureIdx && !isNaN(Number(e.divisions))
            )?.divisions ?? 1;

            let currentVoice = 1;
            let currentTempo = findLast(
                tempoEvents,
                (e) =>
                    e.partIdx === partIdx &&
                    e.measureIdx === measureIdx &&
            ...
            forEach(measureChildren, (measureChild, measureChildIdx) => {
                ...
                if (isMeasureAttributes(measureChild) || isMeasureDirection(measureChild)) {
                    const timeElapsedForPartAndVoice = timeElapsedPerPartAndVoice?.[partIdx]?.[currentVoice] ?? timeElapsedAtStartOfMeasure;
                ...
                } else if (!isMeasureNote(measureChild) || findChildByTagName(measureChild, "rest")) {
                    ...
                    if (durationEl) {
                        ...
                        timeElapsedPerPartAndVoice[partIdx][currentVoice] = timeElapsedForPartAndVoice + eventSeconds;
                    }
                } else {
                    ...
                    if (chordEl) {
                        ...
                        const previousChordDuration = currentChordDurationPerPartAndVoice?.[partIdx]?.[currentVoice] ?? 0;

                        // Progress the time elapsed with the previous chord
                        timeElapsedPerPartAndVoice[partIdx] = timeElapsedPerPartAndVoice?.[partIdx] || {};
                        timeElapsedPerPartAndVoice[partIdx][currentVoice] = timeElapsedForPartAndVoice + previousChordDuration;

                        // Reset the chord duration - we're starting a new chord
                        currentChordDurationPerPartAndVoice[partIdx] = currentChordDurationPerPartAndVoice?.[partIdx] || {};
                        currentChordDurationPerPartAndVoice[partIdx][currentVoice] = 0;
                    }

                    // Extract lyrics
                    const latestVoiceLyricStartIndex = findLastIndex(lyricsEvents, (e) => !e.continuesPrevious && e.partIdx === partIdx && e.voice === measureIdx);
                    ...
                    const latestLyricEvents = hasVoiceLyric
                        ? filter(
                            slice(lyricsEvents, (latestVoiceLyricStartIndex !== -1 ? latestVoiceLyricStartIndex : latestPartLyricStartIndex)),
                            (e) => e.partIdx === partIdx && e.voice === currentVoice
                        )
                        : hasPartLyric
                            ? filter(
                                slice(lyricsEvents, (latestPartLyricStartIndex)),
                                (e) => e.partIdx === partIdx
                            )
                            : [];

                    const previousLyricText = reduce(latestLyricEvents, (acc, e) => acc + (e.continuesPrevious ? "" : " ") + e.lyric, "");
                    let newLyricText = '';
                    ...
                    const currentChordDuration = currentChordDurationPerPartAndVoice?.[partIdx]?.[currentVoice] ?? 0;
                    currentChordDurationPerPartAndVoice[partIdx] = currentChordDurationPerPartAndVoice?.[partIdx] || {};
                    currentChordDurationPerPartAndVoice[partIdx][currentVoice] = Math.max(currentChordDuration, eventSeconds);
                }
            });
        }
    }
