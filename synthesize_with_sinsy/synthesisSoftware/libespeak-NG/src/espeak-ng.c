...
static int SynthCallback(short *wav, int numsamples, espeak_EVENT *events)
{
	...
	while (events->type != 0) {
		if (events->type == espeakEVENT_SAMPLERATE) {
			samplerate_ng = events->id.number;
			samples_split = samples_split_seconds * samplerate_ng;
	...
	if (f_wavfile == NULL) {
		if (samples_split > 0) {
			...
			if (OpenWavFile(fname, samplerate_ng) != 0)
				return 1;
		} else if (OpenWavFile(wavefile, samplerate_ng) != 0)
			return 1;
	}
...
int main(int argc, char **argv)
{
	...
	if (option_waveout || quiet) {
		...
		samplerate_ng = espeak_ng_GetSampleRate();
		samples_split = samplerate_ng * samples_split_seconds;
